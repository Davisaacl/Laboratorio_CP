...
# JPG:      logical. Graphic should be save in jpg format
#
# Example:    savePlots("figure",6,4,TRUE,FALSE,TRUE,FALSE)
#
savePlots <- function(file,width=6,height=6,EPS=TRUE,PDF=TRUE,PNG=FALSE,JPG=FALSE) 
{
    ...
    if(JPG) {
        nfile <- paste(file,"jpg",sep=".")
        dev.copy(jpg,file=nfile,width=100*width,height=100*height)
        dev.off(dev.cur())
    }
}
savePlots <- function(file,width=6,height=6,EPS=TRUE,PDF=TRUE,PNG=FALSE,JPG=FALSE) 
